...
def button_clear():
	...
	return
def button_plus():
	prev_val=entry_widget.get()
	entry_widget.delete(0, END)
	return prev_val
def button_equal(prev_val):
	current_val=entry_widget.get()
	entry_widget.delete(0, END)
	entry_widget.insert(0,int(current_val) + int(prev_val))
	return
...
button_add = Button(root, text='+',padx= 39, pady =10,command=lambda prev_val: button_plus())
button_equal = Button(root, text='=',padx= 91, pady =10,command=lambda: button_equal(prev_val))
